...
try {
    ...
    try {
        ...
        process.send({ msg: 'error', data: 'Unable to find grunt, please install it locally or globably' });
        process.exit(1);
    }
}
...
process.on('message', function (data) {
    ...
    if (data.msg === 'run') {
        ...
        if (!taskDef) {
            return process.send({ msg: 'end', error: 'Task "' + task.name + '" not loaded, specify its directory in the "tasks" grunt config' });
        }
...
process.exit = function (code) {
    ...
    if (code) {
        ...
        // note that this error will be uncaught bellow with process.on('uncaughtException')
        err = new Error('end');
        err.code = code;
        throw err;
    }
