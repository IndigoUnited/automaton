...
var Runner = dejavu.Class.declare({
    ...
    _processQueue: function () {
        ...
        if (!this._inited) {
            this._createWorker();
        // process the next on queue if not empty and the current task is finished
        } else if (!this._current && this._queue.length) {
    ...
    _createWorker: function () {
        ...
        // if the worker exited too early, restart it
